...
public class Sam1949 {
    ...
    public static void search(int x, int y, int leng, Boolean isCutted) {
        ...
        for (int dir = 0; dir < 4; dir++) {
            ...
            if (rand[x][y] <= rand[nx][ny]) {
                if (!isCutted) {
                    int goalCut = rand[nx][ny] - rand[x][y] +1;
                    if (0 < goalCut && goalCut <= K) {
                        int backup = rand[nx][ny];
                        int cutter = rand[nx][ny] - goalCut;
                        visited[nx][ny] = 1;
                        rand[nx][ny] = cutter;
                        search(nx, ny, leng + 1, true);
                        rand[nx][ny] = backup;
                        visited[nx][ny] = 0;
                    }
                }
